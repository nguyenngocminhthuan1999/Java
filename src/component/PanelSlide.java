    ...
    private void login(String userName, String password) {
        th = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Thread.sleep(2000);
                    String sql = "select UserID, UserName, `Password`, `Profile` from `user` where BINARY(UserName)=? limit 1";
                    PreparedStatement p = DatabaseConnection.getInstance().getConnection().prepareStatement(sql);
                    p.setString(1, userName);
                    ResultSet r = p.executeQuery();
                    if (r.first()) {
                        String hashedPasswordFromDB = r.getString("Password");
                        if (BCrypt.checkpw(password, hashedPasswordFromDB)) {
                            ...
                            loading.showError("Password Incorrect");
                        }
                    } else {
                        loading.showError("User not found");
                    }
                    r.close();
                    p.close();
                } catch (InterruptedException e) {
